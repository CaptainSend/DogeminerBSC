...
function approve() {
    ...
    approveDOGE(web3.utils.toWei(trxspenddoc.value));
}
...
function refreshData(){
    ...
    contractBalance(function(result){
        ...
        balanceElem.textContent = stripDecimals(rawStr, 3) + ' DOGE';
    });

    var userBalanceElem = document.getElementById('user-balance');
    userBalance(function(result){
        rawStr = numberWithCommas(Number(result).toFixed(3));
        userBalanceElem.textContent = stripDecimals(rawStr, 3) + ' DOGE';
    });
